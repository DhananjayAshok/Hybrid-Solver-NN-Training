        ...
        for l in range(self.n_layers):
            ...
            for j in range(output_dim):
                for i in range(input_dim):
                    w_b_var_dict[(l, i, j)] = m.addVar(vtype=GRB.CONTINUOUS, name=f"w_{l},{i},{j}")
        ...
        for l in range(self.n_layers):
            ...
            for n in range(batch_size):
                ...
                for j in range(output_dim):
                    ...
                    if l == 0:
                        ...
                        weighted_sum_string = " + ".join([f"self.w_b_var_dict[{(l, i, j)}] * {X[n][i]}"
                                                        for i in range(input_dim)]) +  f" + w_b_var_dict[{(l, j)}]"
                    else:
                    ...
                    if activation:
                        activation_constraint_string = f"inp_out_var_dict[{(n, l, j, 1)}] == " \
                                                       f"max_(inp_out_var_dict[{(n, l, j, 0)}], 0)"
                    else:
                ...
                if l == self.n_layers - 1:
                    ...
                    max_string = "_max("+ ", ".join([f"inp_out_var_dict[{(n, l, j, 1)}]"] for j in output_dim) + ")"
